...
class AgenciaList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'formList_Agencia';
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle('Listagem de agencias');
        ...
        $row3 = $this->form->addFields([new TLabel('Nome agencia:', null, '14px', null)],[$nome_agencia]);
        ...
        $btn_onshow = $this->form->addAction('Cadastrar', new TAction(['AgenciaForm', 'onShow']), 'fa:plus #69aa46');
        ...
        $column_nome_agencia = new TDataGridColumn('nome_agencia', 'Nome agencia', 'left');
        $column_banco_nome_banco = new TDataGridColumn('banco->nome_banco', 'Banco', 'left');
        $column_cidade_descricao = new TDataGridColumn('cidade->descricao', 'Cidade', 'left');
        ...
        $this->datagrid->addColumn($column_nome_agencia);
        $this->datagrid->addColumn($column_banco_nome_banco);
        $this->datagrid->addColumn($column_cidade_descricao);

        $action_onEdit = new TDataGridAction(array('AgenciaForm', 'onEdit'));
        ...
        $action_onDelete = new TDataGridAction(array('AgenciaList', 'onDelete'));
        ...
        $container->add(TBreadCrumb::create(['Cadastros Iniciais','Agencias']));
    ...
    public function onSearch()
    {
        ...
        if (isset($data->nome_agencia) AND ( (is_scalar($data->nome_agencia) AND $data->nome_agencia !== '') OR (is_array($data->nome_agencia) AND (!empty($data->nome_agencia)) )) )
        {

            $filters[] = new TFilter('nome_agencia', 'like', "%{$data->nome_agencia}%");// create the filter 
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for Agencia
