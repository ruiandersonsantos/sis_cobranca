...
class BancoList extends TPage
{
    ...
    private static $formName = 'formList_Banco';
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle('Listagem de bancos');
        ...
        $btn_onshow = $this->form->addAction('Cadastrar', new TAction(['BancoForm', 'onShow']), 'fa:plus #69aa46');
        ...
        $column_nome_banco = new TDataGridColumn('nome_banco', 'Nome banco', 'left');
        $column_nome_banco_completo = new TDataGridColumn('nome_banco_completo', 'Nome banco completo', 'left');
        ...
        $this->datagrid->addColumn($column_nome_banco);
        $this->datagrid->addColumn($column_nome_banco_completo);

        $action_onEdit = new TDataGridAction(array('BancoForm', 'onEdit'));
        ...
        $action_onDelete = new TDataGridAction(array('BancoList', 'onDelete'));
        ...
        $container->add(TBreadCrumb::create(['Cadastros Iniciais','Bancos']));
    ...
    public function onSearch()
    {
        ...
        if (isset($data->nome_banco) AND ( (is_scalar($data->nome_banco) AND $data->nome_banco !== '') OR (is_array($data->nome_banco) AND (!empty($data->nome_banco)) )) )
        {

            $filters[] = new TFilter('nome_banco', 'like', "%{$data->nome_banco}%");// create the filter 
        }

        if (isset($data->nome_banco_completo) AND ( (is_scalar($data->nome_banco_completo) AND $data->nome_banco_completo !== '') OR (is_array($data->nome_banco_completo) AND (!empty($data->nome_banco_completo)) )) )
        {

            $filters[] = new TFilter('nome_banco_completo', 'like', "%{$data->nome_banco_completo}%");// create the filter 
        }
