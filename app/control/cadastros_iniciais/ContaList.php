...
class ContaList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $agencia_id = new TDBUniqueSearch('agencia_id', 'bd_cobranca', 'Agencia', 'id', 'codigo','id asc'  );

        $agencia_id->setMinLength(2);
        $agencia_id->setMask('{codigo}');
        $id->setSize(100);
        ...
        $row3 = $this->form->addFields([new TLabel('Agencia :', null, '14px', null)],[$agencia_id]);
        ...
        $btn_onshow = $this->form->addAction('Cadastrar', new TAction(['ContaForm', 'onShow']), 'fa:plus #69aa46');
        ...
        $column_numero_conta = new TDataGridColumn('numero_conta', 'Numero conta', 'left');
        $column_agencia_codigo = new TDataGridColumn('agencia->codigo', 'Agencia', 'left');
        ...
        $this->datagrid->addColumn($column_agencia_codigo);
        ...
        $action_onDelete = new TDataGridAction(array('ContaList', 'onDelete'));
        ...
        $container->add(TBreadCrumb::create(['Cadastros Iniciais','Contas']));
    ...
    public function onSearch()
    {
        ...
        if (isset($data->numero_conta) AND ( (is_scalar($data->numero_conta) AND $data->numero_conta !== '') OR (is_array($data->numero_conta) AND (!empty($data->numero_conta)) )) )
        {
            ...
            $filters[] = new TFilter('numero_conta', 'like', "%{$data->numero_conta}%");// create the filter 
        }

        if (isset($data->agencia_id) AND ( (is_scalar($data->agencia_id) AND $data->agencia_id !== '') OR (is_array($data->agencia_id) AND (!empty($data->agencia_id)) )) )
        {

            $filters[] = new TFilter('agencia_id', '=', $data->agencia_id);// create the filter 
        }
